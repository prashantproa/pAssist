...
#check: https://www.codechef.com/users/larryzhong 1 starer when playing long
...
documents=os.path.expanduser('~/Documents')+"/data_of_chatapp"
try:os.mkdir(documents)
except FileExistsError:pass
if not os.path.exists(documents+r"\data_for_chatapp.passist"):
    shutil.copyfile(r"data_of_chatapp\data_for_chatapp.passist", os.path.join(documents, os.path.basename(r"data_of_chatapp\data_for_chatapp.passist")))
#pre feeded replies:
file = open(documents+ r"\data_for_chatapp.passist", 'rb')
...
if bgimage=='':bgimage=r"data_of_chatapp\shinchan at night.jpg"
...
temp_img=ImageTk.PhotoImage(Image.open(r"data_of_chatapp\app_icon.png"))
temp=Label(root,image=temp_img,bg='black')
temp.place(relx=.5, rely=.4,anchor= CENTER)
from tkinter import ttk
st=ttk.Style()
#stackoverflow: 'How to change ttk.progressBar color in python'
st.theme_use('clam')
'''
...
st.configure("bar.Horizontal.TProgressbar", troughcolor='black',
                bordercolor='lightblue', background='cyan', lightcolor='cyan', darkcolor='cyan')

temp1=ttk.Progressbar(style="bar.Horizontal.TProgressbar",length=300,mode='determinate',orient='horizontal')
temp1.place(relx=.5, rely=.63,anchor= CENTER)
temp2=Label(root,text="Don't switch to other apps while loading...",fg='lightblue',bg='black')
temp2.place(relx=.5, rely=.66,anchor= CENTER)
temp2.update()
temp1.update()
temp.update()
root.update()
...
for i in range(0,6,1):
    try: temp1.config(value=i);temp1.update(); root.after(10) #try statement to avout progressbar label error when app is closed
    except:sys.exit(1)
    
#root.after(100000)
#--------open & close cortana if not running in bg----------
from pyautogui import *
import pyautogui, psutil,ctypes
...
if not found:
    hotkey('win','c')
    root.after(4000)#open & close cortana on statup(4 sec coz of first time)
    hotkey("alt", "f4")
found=False
#----------------closed--------------------------------
...
for i in range(16,24,1):
    ...
    try: temp1.config(value=i);temp1.update(); root.after(10)
    except:sys.exit(1)
...
import random, webbrowser, os, pyperclip
...
import pyttsx3, textwrap, akinator
engine = pyttsx3.init()
aki = akinator.Akinator()
imported=['pygame','screen_brightness_control', 'tkinter', 'PIL', 'time', 'pyautogui','akinator','listen','say',
          'os', 'pyperclip','random','webbrowser','filedialog','psutil','pickle','pygame','loading','internet',
          'wikipedia','wolframalpha','textwrap','next','downloader','direct','closefile','openfile','search','gs']
psorrsm='Pause'
entries=[]
for i in range(64,80,1):
    try: temp1.config(value=i);temp1.update(); root.after(10)
    except:sys.exit(1)
...
try:
    new_img1 = Image.open(bgimage).resize((w,h))
except:
    bgimage=r"data_of_chatapp\shinchan at night.jpg"
    new_img1 = Image.open(bgimage).resize((w,h))
image1 = ImageTk.PhotoImage(new_img1)
...
def akntr(a):
    ...
    if aki.progression <= 80:  #% showing how close Aki thinks
        ...
        if len(aki.first_guess['name'].split(' '))<2: s=f"{aki.first_guess['name']} {aki.first_guess['description']}"
        ...
        downloader.download(s, limit=1, output_dir=documents, adult_filter_off=False, force_replace=True,
                                timeout=30, verbose=False)
        if os.listdir(documents+'/'+s)==[]:
            shutil.rmtree(documents+'/'+s)
            if s!=f"{aki.first_guess['name']}, {aki.first_guess['description']}":
                s=s+' '+(aki.first_guess['description'])
            downloader.download(s, limit=1, output_dir=documents, adult_filter_off=False, force_replace=True,
                                timeout=30, verbose=False)
        image = Image.open(documents+'/'+s+'/'+os.listdir(documents+'/'+s)[0])#raises indexerror when s folder is empty
        ...
        image.save(fp=documents+"/akinatorimg.png")
        try: shutil.rmtree(documents+'/'+s)  #delete downloaded folder
        except: pass  #if directory not found
        
        akntrimg=ImageTk.PhotoImage(Image.open(documents+"/akinatorimg.png"))
...
def add_image(e=None):
    ...
    bgimage = filedialog.askopenfilename(filetypes = [("Image File", ".jpg .png .jpeg .gif .bmp .tiff .ppm")])
            #copying user selected bgimage to passist location so that loction not found error doesn't occur in case the new bgimage..
            #..is moved to some other place by the user (when new bgimage is set, old one gets deleted from passist location)

    try:shutil.copyfile(bgimage, os.path.join(documents, os.path.basename(bgimage)))
    except shutil.SameFileError:pass
    except:return None
    
    bgimage=documents+'\\'+bgimage[(bgimage[::-1].find(r'/'))*(-1):] #extracting image name (for location)
    #updating image location in data file:
    file = open(documents+"/data_for_chatapp.passist", 'rb')
    data=pickle.load(file)
    data[-1]=bgimage
    file.close()
    file = open(documents+"/data_for_chatapp.passist", 'wb')
...
def set_alarm(text2):
    ...
    text2=text2.replace(' alarm','').replace(' reminder','').replace(' minutes','minute(s)').replace(' hours','hour(s)')
...
def show_reply(reply_text=None,wait=True):
    ...
    if wait=='clearall':
        ...
        reply=reply.replace(' i ',' I ').replace('wolfram|alpha','pAssist')
    ...
    root.update()
    if (i.cget("compound")=="left" or i.cget("justify")=="left") and voice_on==1:#to ensure listen_name() starts again only if last msg is of bot also when voice cmd is on
...
def fld_enter():
    ...
    if q.strip() in['akinator.','play akinator']:
        ...
        if internet():
            akntr_on=True
            show_reply("Hi, Think of a real or fictional character.<n>I will try to guess who it is, reply with:<n>y or 0 for yes<n>n or 1 for no<n>i or 2 for don't know<n>x to exit akinator")
            fld.config(state='disable')
            akntr_ques = aki.start_game()
            show_reply(reply_text=akntr_ques)
...
#By saving photos to a folder outside of program files,
#fix administrator not allowing to:
    #change bg image and
    #save akinator's downloaded image in current folder
